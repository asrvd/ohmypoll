  ...
  return (
    ...
    >
      <div className="flex gap-3 self-start items-center">
        <span className="text-xs text-gray-500 dark:text-gray-300">poll#{id}</span>
      ...
      <div className="flex gap-1 self-start items-center text-gray-500 dark:text-gray-300 text-sm">
        <span className="text-sm text-gray-500 dark:text-gray-300">△ {upvotes}</span>
        {" | "}
        <span className="text-sm text-gray-500 dark:text-gray-300">by {createdBy}</span>
        <span className="text-sm text-gray-500 dark:text-gray-300">
          on {new Date(createdAt).toLocaleDateString()}
        </span>
        {" | "}
        <span className="text-sm text-gray-500 dark:text-gray-300">{votes} {votes === 1 ? 'person' : 'people'} voted</span>
      </div>
    </div>
  );
}
